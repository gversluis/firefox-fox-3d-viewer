...
function buildFallbackScene(chunks) {
  const group = new Group();
...
/**
 * A Group that holds one mesh per animation frame (from a PACK-based VOX file).
 * All frames are invisible except the first one. Use showFrame(n) to animate.
 *
 * @augments Group
 */
class VoxAnimatedGroup extends Group {

	/**
	 * @param {Mesh[]} frames - Array of meshes, one per animation frame.
	 */
	constructor( frames ) {
	...
	/**
	 * Hides the currently visible frame and shows frame n.
	 * The index is clamped to [0, frameCount - 1].
	 *
	 * @param {number} n - Zero-based frame index to show.
	 */
	showFrame( n ) {

		const idx = Math.max( 0, Math.min( Math.floor( n ), this._frames.length - 1 ) );

		if ( idx === this.currentFrame ) return;

		this._frames[ this.currentFrame ].visible = false;
		this._frames[ idx ].visible = true;
...
class VOXLoader extends Loader {
	 ...
	 */
	parse( buffer ) {
		console.log('Vox parse', buffer);
		let expectedModels = 1;
		let hasPack = false;
		...
		while ( i < data.byteLength ) {
			...
			if ( id === 'SIZE' ) {
				console.log("Vox SIZE", chunkSize);
				...
				console.log("Vox XYZI", chunkSize);
				...
				console.log("Vox RGBA", chunkSize);
				palette = [ 0 ];
				...
				console.log("Vox nTRN", chunkSize);
				// Transform Node
				...
				console.log("Vox nGRP", chunkSize);
				// Group Node
				const nodeId = data.getUint32( i, true ); i += 4;
				const attributes = readDict( data, i );
				i += attributes.size;
				...
				console.log("Vox nSHP", chunkSize);
				// Shape Node
				const nodeId = data.getUint32( i, true ); i += 4;
				const attributes = readDict( data, i );
				i += attributes.size;
				...
				console.log("Vox PACK", chunkSize);
				hasPack = true;
				expectedModels = data.getUint32(i, true);
				i += 4;
				console.log("Model count:", expectedModels);

			} else {
				console.log("Vox unknown", id, chunkSize);
				// Skip unknown chunks
				i += chunkSize;
		...
		if ( hasPack ) {
			...
			// PACK files interleave N full model-sets (one per animation frame).
			// chunks.length = expectedModels * modelsPerFrame, so we slice them.
			const modelsPerFrame = chunks.length / expectedModels;
			const frameScenes = [];

			for ( let f = 0; f < expectedModels; f ++ ) {

				// Each frame gets its own slice of chunks; model IDs inside the
				// scene-graph nodes are 0-based relative to that frame's slice.
				const frameChunks = chunks.slice( f * modelsPerFrame, ( f + 1 ) * modelsPerFrame );

				if ( hasNodes ) {

					frameScenes.push( buildObject( 0, nodes, frameChunks ) );

				} else {

					frameScenes.push( buildFallbackScene( frameChunks ) );

				}

			}

			scene = new VoxAnimatedGroup( frameScenes );
			...
			scene = buildFallbackScene( chunks );
...
export { VOXLoader, VoxAnimatedGroup, buildMesh, buildData3DTexture, VOXMesh, VOXData3DTexture };
