...
class DRACOLoader extends Loader {
	...
	parse( buffer, onLoad, onError = ()=>{} ) {
		console.log("Draco loader parse", buffer);
	...
	decodeDracoFile( buffer, callback, attributeIDs, attributeTypes, vertexColorSpace = LinearSRGBColorSpace, onError = () => {} ) {
		console.log("Draco loader decodeDracoFile");
		const taskConfig = {
	...
	decodeGeometry( buffer, taskConfig ) {
		console.log("Draco loader decodeGeometry");
		...
		const geometryPending = this._getWorker( taskID, taskCost )
			.then( ( _worker ) => {
				console.log("Draco loader geometryPending", taskID, taskCost);

				worker = _worker;

				return new Promise( ( resolve, reject ) => {
					console.log("Draco geometryPending promise", taskID, taskCost);
					worker._callbacks[ taskID ] = { resolve, reject };
					console.log("Draco postMessage decode", taskID, taskCost);
		...
		geometryPending
			.catch( (e) => { console.log('geometryPending catch', e); true } )
			.then( () => {
	...
	_createGeometry( geometryData ) {
		console.log('DRACO _createGeometry', geometryData);
	...
	_initDecoder() {
		...
		this.decoderPending = Promise.all( librariesPending )
			.then( ( libraries ) => {
				...
				console.log("Draco workerSourceURL", body);
	...
	_getWorker( taskID, taskCost ) {
		console.log("Draco loader _getWorker");
		return this._initDecoder().then( () => {
			console.log(`Draco loader _initDecoder, workerPool ${this.workerPool.length}, workerLimit ${this.workerLimit}`);
			if ( this.workerPool.length < this.workerLimit ) {
				console.log("DRACO workerSourceURL", this.workerSourceURL);
				const worker = new Worker( this.workerSourceURL );
				console.log('DRACO CREATED WORKER', worker);

				worker._callbacks = {};
				worker._taskCosts = {};
				worker._taskLoad = 0;

				console.log('DRACO WORKER BEFORE POSTMESSAGE INIT');
				worker.postMessage( { type: 'init', decoderConfig: this.decoderConfig } );
				console.log('DRACO WORKER AFTER POSTMESSAGE INIT');
				worker.onmessage = function ( e ) {
					console.log('DRACO WORKER ONMESSAGE', e.data);
					...
					switch ( message.type ) {
						...
						case 'decode':
							console.log('DRACO WORKER ONMESSAGE decode', message, worker._callbacks[ message.id ]);
			...
			console.log('Draco taskCosts', worker._taskCosts);
			return worker;
...
function DRACOWorker() {
	...
	onmessage = function ( e ) {
		...
		switch ( message.type ) {
			...
			case 'decode':
				console.log("DRACO worker decode");
	...
	function decodeGeometry( draco, decoder, array, taskConfig ) {
		console.log("DRACO worker decodeGeometry");
	...
	function decodeIndex( draco, decoder, dracoGeometry ) {
		console.log("DRACO worker decodeIndex");
	...
	function decodeAttribute( draco, decoder, dracoGeometry, attributeName, TypedArray, attribute ) {
		console.log("DRACO worker decodeAttribute");
	...
	function getDracoDataType( draco, TypedArray ) {
		console.log("DRACO worker getDracoDataType");
		switch ( TypedArray ) {
