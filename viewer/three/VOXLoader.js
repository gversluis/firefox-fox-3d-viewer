/* 
Specs/sample files @ https://github.com/ephtracy/voxel-model/tree/master/

2026-04-16	Added support for PACK header/setting frames - Gerben Versluis
*/
...
class VOXLoader extends Loader {
	...
	parse( buffer ) {
		...
		while ( i < data.byteLength ) {
			...
			i += 4; // ignore num bytes of children chunks
			...
			if ( id === 'SIZE' ) {
				...
				chunks.push( chunk );
				if (chunks.length>expectedModels) console.warn(`Found ${chunks.length} models, while ${expectedModels} were announced in PACK header`);
				i += chunkSize - ( 3 * 4 );

			} else if ( id === 'XYZI' ) {
				// console.log("Vox XYZI", chunkSize);
				const numVoxels = data.getUint32( i, true ); i += 4;
				chunks[chunks.length-1].data = new Uint8Array( buffer, i, numVoxels * 4 );

				i += numVoxels * 4;

			} else if ( id === 'RGBA' ) {	// overwrite palette
				...
				const bytes = new Uint8Array(buffer, i, chunkSize);
				console.log("Vox unknown", id, chunkSize, bytes);
				// Skip unknown chunks
				i += chunkSize;
		...
		chunks.forEach( chunk => chunk.palette ??= palette );
		
		// Build Three.js scene graph from nodes
		let scene;

		if ( Object.keys( nodes ).length > 0 ) {
			// console.log(`Vox has ${Object.keys( nodes ).length} nodes`);
			scene = buildObject( 0, nodes, chunks );
		} else {
			// console.log(`Vox has ${chunks.length} chunks`);
		...
		// add setFrame method for Vox files with PACK
		scene.setFrame = function(frame) {
			console.log('scene setFrame',this);
			if (frame>=this.children.length) {
				console.warn(`Requested non-existing frame ${frame}`);
				return this.children.length;
			}
			if (this.activeFrame) {
				if (this.activeFrame == frame) return;	// nothing to do
				this.children[this.activeFrame].visible = false;
  			} else {
			...
			console.log('new activeFrame', this.activeFrame);
			this.children[this.activeFrame].visible = true;
			return this.children.length;
		};
